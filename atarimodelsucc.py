...
# path needs a folder results with the python file
STORE_PATH = '/results'
 # some variables
MAX_EPSILON = 1
MIN_EPSILON = 0.01
...
MIN_BETA = 0.4
MAX_BETA = 1.0
NUM_FRAMES = 4 #fixed for atari systems
...
class DQModel(keras.Model):
    def __init__(self, hidden_size: int, num_actions: int, dueling: bool):
        ...
        self.adv_dense2 = tf.keras.layers.Dense(hidden_size, activation='relu',use_bias=False,kernel_initializer=keras.initializers.he_normal())
        ...
        self.adv_outb = tf.keras.layers.Dense(num_actions,activation='softmax',kernel_initializer=keras.initializers.he_normal())
        if dueling:
...
# traning start with keyboard options
num_episodes = 100000 # could be higher but for system sakes less
eps = MAX_EPSILON
render = True
